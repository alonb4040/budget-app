...
export const LOAN_FLD_STYLE: React.CSSProperties = { width:"100%", boxSizing:"border-box", background:"var(--surface2)", border:"1px solid var(--border)", borderRadius:8, padding:"8px 10px", color:"var(--text)", fontSize: 14, fontFamily:"inherit", outline:"none" };
  ...
  return (
    <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:12 }}>
      {[["lender","שם המלווה"],["start_date","תאריך התחלה","date"],["end_date","תאריך סיום","date"],["amount","סכום ראשוני (₪)","number"],["monthly","החזר חודשי (₪)","number"]].map(([k,lbl,t]) => (
        <div key={k}>
          <div style={{ fontSize: 13, color:"var(--text-dim)", marginBottom:3 }}>{lbl}</div>
          <input type={t||"text"} value={f[k]||""} onChange={e=>set(k,e.target.value)} style={LOAN_FLD_STYLE} placeholder="..." />
        </div>
      ))}
      <div>
        <div style={{ fontSize: 13, color:"var(--text-dim)", marginBottom:3 }}>ריבית</div>
      ...
      <div style={{ gridColumn:"1/-1" }}>
        <div style={{ fontSize: 13, color:"var(--text-dim)", marginBottom:3 }}>הערות</div>

...
export default function ClientApp({ session, onLogout }) {
  ...
  const startAnalysis = async () => {
    ...
    try {
      ...
      showToast("שגיאה: " + e.message); setScreen("upload");
    }
  };
  ...
  const confirmRemember = async () => {
    ...
    await supabase.from("remembered_mappings").upsert(
      { client_id: session.id, business_name: bizName, category: cat },
      { onConflict: "client_id,business_name" }
    );
    const updated = { ...rememberedMappings, [bizName]: cat };
    setRememberedMappings(updated);
    // Apply to all same-name tx
    let count = 0;
    setTransactions(p => p.map(t => {
      if (t.name === bizName && t.id !== pendingRemember.txId) { count++; return { ...t, cat, edited: cat !== t.originalCat, conf: "high" }; }
      return t;
    }));
    showToast(`✅ נזכר! ${bizName} → ${cat}${count > 0 ? ` (עודכן ב-${count + 1} עסקאות)` : ""}`);
    setPendingRemember(null);
  };

  // ── SAVE SUBMISSION ──
  const saveSubmission = async () => {
    const label = uploadedFiles.map(f => f.name.replace(/\.[^.]+$/, "")).join(" + ");
  ...
  const filteredTx = transactions.filter(t => {
    ...
    if (search) {
      ...
      if (!t.name.toLowerCase().includes(s) && !t.cat.toLowerCase().includes(s)) return false;
    }
    return true;
  });

  // ── CHART ──
  const chartData = submissions.slice(0, 6).reverse().map(s => ({
    name: new Date(s.created_at).toLocaleDateString("he-IL", { day: "numeric", month: "short" }),
    הוצאות: Math.round((s.transactions || []).reduce((sum, t) => sum + t.amount, 0))
  }));
  ...
  const summaryGroups = () => {
    ...
    Object.entries(CATEGORIES).forEach(([group, items]) => {
      items.forEach(item => {
        const txs = transactions.filter(t => t.cat === item);
        if (!txs.length) return;
        if (!result[group]) result[group] = {};
        result[group][item] = txs;
      });
    });
    return result;
  };
  ...
  if (loadingData) return (
    <div style={{ background: C.bg, minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ textAlign: "center" }}><Spinner /><div style={{ marginTop: 16, color: C.dim }}>טוען...</div></div>
    </div>
  );
    ...
    <div style={{ background: C.bg, minHeight: "100vh", color: "#e8eaf6", fontFamily: "'Heebo', sans-serif" }}>
      ...
      <div style={{ background: C.surface, borderBottom: `1px solid ${C.border}`, padding: "14px 24px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            <div style={{ fontWeight: 700, fontSize: 14 }}>מאזן חכם</div>
            <div style={{ fontSize: 11, color: C.dim }}>שלום, {session.name}</div>
          </div>
        </div>
      ...
      {screen === "dashboard" && (
        <div style={{ maxWidth: 960, margin: "0 auto", padding: "28px 20px" }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(170px,1fr))", gap: 12, marginBottom: 24 }}>
            <KpiCard icon="📁" label="הגשות" value={submissions.length} />
            ...
            <Card style={{ marginBottom: 24 }}>
              ...
              <ResponsiveContainer width="100%" height={180}>
                <BarChart data={chartData}>
                  ...
                  <Tooltip contentStyle={{ background: C.surface2, border: `1px solid ${C.border}`, borderRadius: 8, color: "#e8eaf6", fontFamily: "'Heebo'" }} />
                  <Bar dataKey="הוצאות" fill={C.accent} radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </Card>
          )}

          <Btn style={{ marginBottom: 24 }} onClick={() => { setUploadedFiles([]); setTransactions([]); setScreen("upload"); }}>
            📂 העלה קבצים חדשים
          </Btn>

          <div style={{ fontWeight: 700, marginBottom: 12 }}>היסטוריית הגשות</div>
          {submissions.length === 0 ? (
            <Card style={{ textAlign: "center", padding: 48, color: C.dim }}>
              <div style={{ fontSize: 36, marginBottom: 12 }}>📂</div>
              <div>טרם הועלו קבצים</div>
            </Card>
          ) : submissions.map(s => {
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 10 }}>
                  <div>
                    ...
                    <div style={{ display: "flex", gap: 8, marginTop: 8, flexWrap: "wrap" }}>
                      {top3.map(([cat, amt]) => (
                        <span key={cat} style={{ background: C.surface2, border: `1px solid ${C.border}`, borderRadius: 8, padding: "3px 10px", fontSize: 11 }}>
      ...
      {screen === "upload" && (
        <div style={{ maxWidth: 640, margin: "0 auto", padding: "28px 20px" }}>
          <Card style={{ marginBottom: 16 }}>
            <div style={{ fontWeight: 700, fontSize: 16, marginBottom: 4 }}>📁 העלאת קבצי עסקאות</div>
            ...
            <div
              ...
              style={{ border: `2px dashed ${C.border}`, borderRadius: 12, padding: "40px 24px", textAlign: "center", cursor: "pointer", background: C.surface2 }}
            >
              ...
              {uploadedFiles.map(f => (
                <div key={f.name} style={{ background: "rgba(46,204,138,0.07)", border: "1px solid rgba(46,204,138,0.2)", borderRadius: 10, padding: "10px 14px", display: "flex", alignItems: "center", gap: 12 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 11, color: C.dim }}>{(f.size / 1024).toFixed(0)} KB</div>
                  </div>
      ...
      {screen === "loading" && (
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "60vh", gap: 16 }}>
          <Spinner size={52} />
          <div style={{ fontSize: 16, fontWeight: 600 }}>מנתח עסקאות...</div>
          <div style={{ fontSize: 13, color: C.dim }}>מסווג כל עסקה לסעיף המתאים</div>
        </div>
      )}
      ...
      {screen === "review" && (
        <div style={{ maxWidth: 1100, margin: "0 auto", padding: "20px" }}>
          <Card style={{ marginBottom: 14 }}>
            <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 2 }}>✏️ עריכה ואישור עסקאות</div>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap", alignItems: "center" }}>
            ...
            {[["all","הכל"], ["low","⚠️ לבדיקה"], ["edited","✏️ נערך"]].map(([f, label]) => (
              <button key={f} onClick={() => setFilter(f)} style={{ padding: "6px 14px", borderRadius: 20, fontSize: 12, fontFamily: "inherit", cursor: "pointer", border: `1px solid ${filter === f ? C.accent : C.border}`, background: filter === f ? "rgba(79,142,247,0.12)" : C.surface2, color: filter === f ? C.accent : C.dim }}>
                {label}
              </button>
            ))}
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                <thead>
                  <tr style={{ background: C.surface2 }}>
                    {["תאריך","שם בית עסק","סכום","סעיף","ביטחון","מקור"].map(h => (
                      <th key={h} style={{ padding: "10px 14px", textAlign: "right", fontSize: 11, fontWeight: 600, color: C.dim, borderBottom: `1px solid ${C.border}` }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {filteredTx.map(tx => (
                    <tr key={tx.id} style={{ background: tx.edited ? "rgba(247,201,72,0.04)" : "transparent" }}>
                      ...
                      <td style={{ padding: "9px 14px", borderBottom: `1px solid ${C.border}22` }}>
                        ...
                        {tx.maxCat && <div style={{ fontSize: 11, color: C.dim }}>{tx.maxCat}</div>}
                      </td>
                      ...
                      <td style={{ padding: "9px 14px", borderBottom: `1px solid ${C.border}22` }}>
                        <div
                          onClick={() => { setActiveTxId(tx.id); setCatSearch(""); setCatPanelOpen(true); }}
                          style={{ background: C.surface2, border: `1px solid ${tx.edited ? C.yellow : C.border}`, borderRadius: 8, padding: "6px 10px", cursor: "pointer", fontSize: 12, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 6, minWidth: 180, maxWidth: 220 }}
                        >
                          <span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{tx.cat}</span>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
            <div style={{ fontSize: 13, color: C.dim }}>
              {transactions.length} עסקאות · סה"כ: <strong style={{ color: C.red }}>₪{transactions.reduce((s, t) => s + t.amount, 0).toLocaleString()}</strong>
            </div>
            <div style={{ display: "flex", gap: 10 }}>
              <Btn variant="ghost" onClick={() => setScreen("upload")}>← חזור</Btn>
              <Btn onClick={saveSubmission}>💾 שמור והפק מאזן ←</Btn>
            </div>
          </div>
        </div>
      )}
      ...
      {screen === "summary" && (
        <div style={{ maxWidth: 960, margin: "0 auto", padding: "28px 20px" }}>
          <Card style={{ marginBottom: 20 }}>
            <div style={{ fontWeight: 700, fontSize: 16, marginBottom: 16 }}>📋 סיכום מאזן</div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(150px,1fr))", gap: 12 }}>
              {[
                { l: "סה\"כ הוצאות", v: "₪" + transactions.reduce((s, t) => s + t.amount, 0).toLocaleString(), c: C.red },
                { l: "עסקאות", v: transactions.length },
                { l: "קטגוריות", v: new Set(transactions.map(t => t.cat)).size },
                { l: "לבדיקה", v: transactions.filter(t => t.conf === "low" && !t.edited).length, c: C.yellow },
              ].map(k => (
                <div key={k.l} style={{ background: C.surface2, borderRadius: 10, padding: "14px 16px" }}>
                  <div style={{ fontSize: 11, color: C.dim, marginBottom: 4 }}>{k.l}</div>
          ...
          {Object.entries(summaryGroups()).map(([group, items]) => {
            ...
            return (
              <Card key={group} style={{ marginBottom: 12, padding: 0, overflow: "hidden" }}>
                <div style={{ padding: "12px 16px", background: C.surface2, display: "flex", justifyContent: "space-between", fontWeight: 700, fontSize: 13 }}>
                  <span>{group}</span>
                  <span style={{ color: C.red }}>₪{Math.round(total).toLocaleString()}</span>
                </div>
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                  <tbody>
                    {Object.entries(items).map(([cat, txs]) => (
                      <tr key={cat}>
                        <td style={{ padding: "8px 16px", borderBottom: `1px solid ${C.border}22` }}>{cat}</td>
                        <td style={{ padding: "8px 16px", borderBottom: `1px solid ${C.border}22`, color: C.red, fontWeight: 600 }}>₪{Math.round(txs.reduce((s, t) => s + t.amount, 0)).toLocaleString()}</td>
                        <td style={{ padding: "8px 16px", borderBottom: `1px solid ${C.border}22`, color: C.dim }}>{txs.length} עסקאות</td>
                        <td style={{ padding: "8px 16px", borderBottom: `1px solid ${C.border}22`, color: C.dim, fontSize: 11 }}>{[...new Set(txs.map(t => t.name))].slice(0, 2).join("، ")}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <div style={{ position: "fixed", bottom: 0, left: "50%", transform: "translateX(-50%)", width: 320, background: C.surface, border: `1px solid ${C.border}`, borderRadius: "14px 14px 0 0", boxShadow: "0 -8px 40px rgba(0,0,0,0.6)", zIndex: 9999, display: "flex", flexDirection: "column", maxHeight: 420 }}>
            ...
            <div style={{ overflowY: "auto", flex: 1 }}>
              {Object.entries(CATEGORIES).map(([group, items]) => {
                ...
                const matched = items.filter(item => !q || item.toLowerCase().includes(q) || group.toLowerCase().includes(q));
                if (!matched.length) return null;
                const currentCat = transactions.find(t => t.id === activeTxId)?.cat;
                return (
                  <div key={group}>
                    <div style={{ padding: "5px 12px 2px", fontSize: 10, fontWeight: 700, color: C.dim, letterSpacing: "0.5px" }}>{group}</div>
                    {matched.map(item => (
                      <div
                        key={item}
                        onMouseDown={e => { e.preventDefault(); applyCategory(activeTxId, item); }}
                        style={{ padding: "8px 14px", fontSize: 12, cursor: "pointer", color: item === currentCat ? C.green : "#e8eaf6", fontWeight: item === currentCat ? 700 : 400 }}
                        onMouseEnter={e => e.currentTarget.style.background = "rgba(79,142,247,0.1)"}
                        onMouseLeave={e => e.currentTarget.style.background = "transparent"}
                      >
                        {item === currentCat ? "✓ " : ""}{item}
                      </div>
                    ))}
                  </div>
                );
        ...
        <>
          ...
          <div style={{ position: "fixed", bottom: 80, left: "50%", transform: "translateX(-50%)", background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: "14px 18px", boxShadow: "0 8px 32px rgba(0,0,0,0.5)", zIndex: 9997, maxWidth: 380, width: "90%" }}>
            ...
            <div style={{ fontSize: 12, color: C.dim, marginBottom: 12 }}>
              לשייך <strong style={{ color: "#e8eaf6" }}>{pendingRemember.bizName}</strong> תמיד לסעיף <strong style={{ color: C.accent }}>{pendingRemember.cat}</strong>?
            </div>
            <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
              <Btn variant="ghost" size="sm" onClick={() => setPendingRemember(null)}>לא תודה</Btn>
              <Btn size="sm" onClick={confirmRemember}>כן, זכור 👍</Btn>
            </div>
          </div>
        </>
      )}
      ...
      {/* ── MINI TOAST ── */}
      {toast && (
        <div style={{ position: "fixed", bottom: 24, left: "50%", transform: "translateX(-50%)", background: C.green, color: "#000", padding: "9px 20px", borderRadius: 20, fontSize: 13, fontWeight: 600, zIndex: 99999, whiteSpace: "nowrap" }}>
          {toast}
        </div>
      )}
    </div>
